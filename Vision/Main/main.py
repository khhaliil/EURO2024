...
cv2.createTrackbar("X Correction", "Trackbars", 850, 1000, nothing)  # 50 is the initial value
######################################### TRACKING ############################################################
...
def update_tracking_data(tag_to_track):
    ...
    for tag in tags_to_track:
        ...
        if current_position is not None:
            global_current_position = pixel_to_mm(current_position[0], current_position[1])
        ...
        if ennemy_robot_predictions is not None:
            global_enemy_robot_predictions = [pixel_to_mm(pos[0], pos[1]) for pos in ennemy_robot_predictions]
...
while True : 
    ...
    centers = Model.perform_detection(yolo_model, img_top_down,0.970)
    for (center_x, center_y), confidence, class_name in centers:
        cv2.circle(img_top_down, (center_x, center_y), 5, (0, 0, 255), -1)
        label = f'{class_name} {confidence:.2f}'
        cv2.putText(img_top_down, label, (center_x + 10, center_y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
